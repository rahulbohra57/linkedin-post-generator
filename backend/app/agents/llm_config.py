...
Set PRIMARY_LLM in your .env to choose the model for creative/reasoning tasks:
  PRIMARY_LLM=huggingface → Qwen2.5-7B via HuggingFace router (uses HF token)  [default]
...
# Which model to use for creative tasks. Default: huggingface.
...
def get_gemini_fast() -> LLM:
    """Gemini 2.0 Flash — secondary LLM for fast formatting/classification tasks."""
    return LLM(
        model="gemini/gemini-2.0-flash",
        api_key=settings.gemini_api_key,
        temperature=0.3,
...
def get_gemma_primary() -> LLM:
    """Gemma 3 4B via Google AI Studio (free tier, 30 RPM)."""
    return LLM(
...
def primary() -> LLM:
    ...
    Controlled by PRIMARY_LLM env var (default: huggingface).
    """
    global _primary_llm
    if _primary_llm is None:
        if _PRIMARY == "huggingface" and _HF_TOKEN:
            ...
            raise RuntimeError("No LLM API key configured. Set HUGGINGFACEHUB_API_TOKEN or GEMINI_API_KEY.")
    return _primary_llm
